...
const TrendingBar = (props) => {
  ...
  return (
    <>
      <div className="w-full h-7 flex justify-between items-start">
        <h2 className="uppercase text-white text-sm ">trending</h2>
        <div className="relative flex flex-col items-end gap-1 transition-all">
          <button
            onClick={() => setOpen(!open)}
            className="w-20 px-[0.1rem] py-[0.01rem] text-[10px] flex items-center justify-center gap-1 bg-stone-900 text-white rounded-sm border-stone-800 border hover:bg-stone-800 cursor-pointer"
          >
          ...
          </button>
          {
            <div
              ref={dropDownRef}
              className={` w-20 rounded bg-stone-900 border border-stone-800 transition-all duration-300 ${open ? "opacity-100 scale-100 pointer-events-auto z-1" : "opacity-0 scale-95 pointer-events-none -z-10"}`}
            >
