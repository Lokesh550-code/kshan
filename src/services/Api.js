...
export const getTop = async () => {
  try {
    ...
    const res = [...movie.data.results.map(item => ({...item, media_type: "Movie"})), ...tv.data.results.map(item => ({...item, media_type: "TV"}))];
...
export const getCritiicallyAcclaimed = async () => {
  try {
    const [movie, tv] = await Promise.all([
      ...
      api.get(`/discover/tv?include_adult=false&vote_average.gte=7.5&vote_count.gte=5000&first_air_date.gte=1990-01-01&sort_by=vote_average.desc`)
    ]);

    const crit = [...movie.data.results.map(item => ({...item, media_type: "Movie"})), ...tv.data.results.map(item => ({...item, media_type: "TV"}))]
